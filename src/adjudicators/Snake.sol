...
contract SnakeGame is IAdjudicator {
    ...
    function calculateNextState(
        ...
        for (uint i = 0; i < 2; i++) {
            ...
            bool ateFood = false;
            for (uint j = 0; j < nextState.food.length; j++) {
                if (pointsEqual(newHead, nextState.food[i])) {
                    ateFood = true;
                    break;
                }
            }
            ...
            moveSnake(nextState.snakes[i], newHead, ateFood);
            ...
            for (uint j = 0; j < 2; j++) {
                ...
                for (uint k = 0; k < prevState.snakes[j].body.length; k++) {
                    // Cannot collide with its own head
                    if (k == 0 && i == j) {
                        continue;
                    }

                    if (pointsEqual(newHead, prevState.snakes[j].body[k])) {
                        nextState.snakes[i].isDead = true;
                        break;
                    }
                }
    ...
    function moveSnake(Snake memory snake, Point memory newHead, bool ateFood) internal pure {
        if (ateFood) {
    ...
    function statesEqual(
        ...
        // TODO: Since food is randomly placed, we can't compare it directly
        // but do we need to compare it at all?
